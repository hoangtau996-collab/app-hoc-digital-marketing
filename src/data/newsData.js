...
/* ------------------------------------------------------------------ *
 * TIN THẬT - do máy cập nhật, KHÔNG sửa tay trong file này            *
 * ------------------------------------------------------------------ */

/**
 * Tin thật nằm ở realNews.json chứ không viết thẳng vào đây.
 *
 * VÌ SAO TÁCH RA: quy trình tự động chạy hằng ngày (.github/workflows)
 * phải ghi đè danh sách này. Cho máy sửa thẳng file JavaScript có chú
 * thích là cách chắc chắn có ngày mất hết chú thích hoặc hỏng cú pháp.
 * Để ở JSON thì máy chỉ việc ghi lại cả file, không cần hiểu cú pháp.
 *
 * Phần tin mô phỏng bên dưới thì ngược lại: người viết, máy không đụng.
 */
// `with { type: 'json' }` là bắt buộc. Bộ đóng gói Vite bỏ qua cũng chạy, nhưng
// Node thuần thì ném lỗi nếu thiếu — mà các script kiểm tra dữ liệu và quy trình
// tự động đều chạy bằng Node thuần.
import REAL_SOURCED_NEWS from './realNews.json' with { type: 'json' };
