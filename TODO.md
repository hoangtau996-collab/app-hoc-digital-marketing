...
- [ ] **⚠️ Deploy LẠI `firestore.rules` — bản trên máy chủ đang cũ hơn kho mã.** Thiếu hai phần: luật cho collection `support_messages` (chưa deploy thì Hộp Thư Hỗ Trợ luôn báo "Chưa đọc được hộp thư") và ràng buộc id tài liệu trong `canCreateProfile` (chưa deploy thì lỗ chiếm chỗ hồ sơ học viên vẫn mở). **Phải kiểm chứng ngay sau khi Publish** vì phần thứ hai có thể chặn nhầm học viên mới: [DEPLOYMENT.md](DEPLOYMENT.md#security-rules--bắt-buộc-deploy).
